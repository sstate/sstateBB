      ...
      if (collection.length > 0){
        ...
        auth.makePasswordHash(options, function(passwordHash){
          if (passwordHash === userObject.get('password')){
            ...
            request.session.set('username', userObject.get('username'));
            request.session.set('user_role', userObject.get('user_role'));
            request.session.set('status', userObject.get('status'));
            reply.redirect('/');
          }else {
